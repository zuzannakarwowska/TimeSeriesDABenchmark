...
# Load data
counts <- read_csv("mesnage_dataset/ready_mesnage_counts.csv") %>%
  column_to_rownames(var = colnames(.)[1])

metadata <- read_csv("mesnage_dataset/ready_mesnage_metadata.csv") %>%
  column_to_rownames(var = colnames(.)[1])
...
ggsave(
  filename = "/Users/zkarwowska/Desktop/mesnage_logFC.pdf",
  plot = g,
  width = 5,
  height = 7,
  units = "in",
