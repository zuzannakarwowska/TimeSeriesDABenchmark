...
results_df <- bind_rows(
  ...
  input_df <- combined_data %>% select(ends_with('_ypred'))
  ...
  pcoa_plot <- ggplot(pcoa_scores, aes(x = PC1, y = PC2)) +
    ...
    theme(
      ...
      axis.title.y = element_text(size = 18),  # Change y-axis title size
      ...
      legend.title = element_text(size = 18),  # Change legend title size      
