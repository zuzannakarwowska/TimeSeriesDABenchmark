...
new_df <- mesnage_res_mean %>% select(-c('sample_size', 'rep')) / 179
new_df$sample_size <- mesnage_res_mean$sample_size
new_df$rep <- mesnage_res_mean$rep
...
pngfile <- fs::path(knitr::fig_path(), "Rplot.pdf")
ggsave(
  filename = pngfile,
  plot = g,
  width = 10,
...
# Alternative: Save as PNG with 10x5 inch dimensions
pngfile_png <- fs::path(knitr::fig_path(), "Rplot.png")
agg_png(pngfile_png, width = 10, height = 5, units = "in", res = 300)
plot(g)
invisible(dev.off())
