...
run_zigmm <- function(counts, metadata){
  ...
  nbmm_counts <- nbmm.input.df %>% dplyr::select(-(Sample_ID:Timepoint)) 
...
otu_table <- read.csv('ready_mesnage_counts.csv', row.names = 'X') # sample id in columns
metadata <- read.csv('ready_mesnage_metadata.csv', row.names = 'X')
absolute_counts <- read.csv('ready_mesnage_absolute.csv', row.names = 1) #bacteria as columns

rownames(metadata) <- metadata$Sample_ID

#linda_predictions <- run_linda(otu_table[1:220, ],  metadata)
#zigmm_predictions <- run_zigmm(t(absolute_counts), metadata)
#maaslin_predictions <- run_maaslin(otu_table, metadata)

results <- data.frame()

for (rep in seq(1, 5)){
  for (sample_size in c(10, 20, 30, 40, 50, 60, 70)) {
    ...
    absolute_counts_small <- absolute_counts %>% dplyr::filter(rownames(absolute_counts) %in% small_metadata$Sample_ID)
    
    # Run models
    linda_predictions   <- run_linda(small_otu_table[1:218, ], small_metadata)
    maaslin_predictions <- run_maaslin(small_otu_table, small_metadata)
    zigmm_predictions   <- run_zigmm(t(absolute_counts_small), small_metadata)
...
write.csv(results, 'mesnage_results.csv')
