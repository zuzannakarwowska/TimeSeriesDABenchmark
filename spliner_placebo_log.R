...
pckgs <- c('dplyr', 'data.table', 'splinectomeR', 'foreach', 'doParallel', 'compositions') 
...
x <- foreach(sample_size=SampleSizes, .packages = pckgs, .verbose = TRUE) %:%
  ...
  foreach(rep=Reps, .packages = pckgs, .verbose = FALSE) %dopar% {
    ...
    output_path <- paste0('/g/scb/zeller/karwowsk/zeevi_dataset_v5//results/trendyspliner/placebo/relab/')
    ...
    subsampled_counts <- master_counts %>%
      dplyr::select(all_of(master_metadata$sample_name))
    
    # Change to Log Relative Abundance
    subsampled_counts_relab <- apply(subsampled_counts, 2, function(x) x / sum(x)) %>% 
      + 1e-3 %>%
      log() %>% 
      t() %>% 
      as.data.frame() 
    
    subsampled_counts_relab$sample_name <- colnames(subsampled_counts)
    merged_df <- merge(subsampled_counts_relab, master_metadata, by='sample_name', all.x=TRUE)
