...
model_name <- 'metasplines'
MEASURES <- data.frame()
transformation <- 'none'

for (N in c(seq(10, 80))) {
  for (efs in c(125, 15, 2, 3, 5)){
    for (rep in seq(10)) { 
      for (sampling in c(2, 4, 6)){
        wd <- glue('/Users/zkarwowska/new_embl_folder/zeevi_dataset_v5/results/{model_name}/one_arm/')
        #wd <- glue('/Users/zkarwowska/new_embl_folder/zeevi_dataset_v5/results/trendyspliner/one_arm/')
        
        
        if (transformation == 'none'){
          file <- glue('{wd}/{model_name}_sampling_{N}_efs_{efs}_rep_{rep}_d_{sampling}.csv')
          
        } else{
          file <- glue('{wd}/{transformation}/{model_name}_sampling_{N}_efs_{efs}_rep_{rep}_d_{sampling}.csv')
          #file <- glue('{wd}/{transformation}/spliner_sampling_{N}_efs_{efs}_rep_{rep}_d_{sampling}.csv')
